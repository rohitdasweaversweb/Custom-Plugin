jQuery(document).ready(function (jQuery) {
            ...
            success: function (response) {
                ...
                if (response.success && response.data.images && response.data.images.length > 0) {
                    response.data.images.forEach((item, index) => {
                        ...
                        let imageHtml = `<img src="${item.file_names[item.file_names.length - 1]}" width="100" />`;
                        ...
                        <tr>
                            ...
                            <span class="shortcode copy-shortcode" data-gallery-id="${item.id}">[gallery_custom id="${item.id}"]</span>
                        </td>
                        </tr>
                    `);
    ...
    // Add click-to-copy functionality for the shortcodes
    jQuery(document).on('click', '.copy-shortcode', function () {
        const shortcodeText = jQuery(this).text();

        // Use Clipboard API to copy text
        navigator.clipboard.writeText(shortcodeText).then(() => {
            // Show confirmation message
            jQuery('#sts-message').html('<p style="color:green;">Shortcode copied to clipboard!</p>');
            setTimeout(function () {
                jQuery('#sts-message').html(''); // Clear the message after 3 seconds
            }, 3000);
        }).catch(err => {
            console.error('Failed to copy shortcode: ', err);
            jQuery('#sts-message').html('<p style="color:red;">Failed to copy shortcode.</p>');
            setTimeout(function () {
                jQuery('#sts-message').html(''); // Clear the message after 3 seconds
            }, 3000);
        });
    ...
    function generatePagination(totalPages, currentPage) {
        ...
        if (totalPages <= 1) {
            pagination.hide(); // Hide pagination if there is only one page
            return;
        }
    ...
    jQuery(document).on('click', '.delete-btn', function () {
        ...
        if (confirm('Are you sure you want to delete this image?')) {
            jQuery.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        // Check the updated total pages after deletion
                        const totalPages = response.data.totalPages;

                        // If the current page is now empty, move to the previous page if possible
                        if (currentPage > totalPages) {
                            currentPage = totalPages; // Set to the last available page
                        }

                        // Reload the gallery on the appropriate page
                        loadGallery(currentPage);

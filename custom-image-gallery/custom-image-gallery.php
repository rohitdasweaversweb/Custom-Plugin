...
function delete_gallery_image() {
    ...
    // Perform deletion
    $result = $wpdb->delete($wp_gallery, array('id' => $id));
    
    if ($result) {
        // Get the new total number of images
        $total_images = $wpdb->get_var("SELECT COUNT(*) FROM $wp_gallery");
        $limit = 2; // Same limit as used in pagination
        $total_pages = ceil($total_images / $limit);
        
        // Return the success message and updated total pages
        wp_send_json_success(array(
            'message' => 'Image deleted successfully.',
            'totalImages' => $total_images,
            'totalPages' => $total_pages // Return the new total number of pages
        ));
    } else {
